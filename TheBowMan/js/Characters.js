...
export const BowManScore = {
	...
	init:function(screen){
		this.WIDTH = (12*screen.width/100);
		this.HEIGHT = (6*screen.height/100);
...
export const BowManLife = {
	...
	cntnr: null,
	init:function(screen){
		this.WIDTH = (5*screen.width/100)*this.LIVES;
		this.HEIGHT = (7*screen.height/100);
...
function Arrow(headX,headY,angle)
{
	...
	this.rect = () => {
		return Dmsns(this.hx-20,this.hy-3,20,6);
	}
	...
	this.render = (delta) => {
		...
		if (this.shot == true)
		{
			this.tx+=this.speed*delta*Math.cos(-this.angle);
			this.ty-=this.speed*delta*Math.sin(-this.angle);

			this.displace(0,0,this.angle);
		}
		this.mid.render();
		this.front.render();
		this.back.render();
...
export const Bow = {
	...
	init:function(screen){
		...
		this.R0.X1 = BowMan.X+BowMan.WIDTH;
		this.R0.Y1 = BowMan.Y;
		this.R0.X2 = BowMan.X+BowMan.WIDTH;
		this.R0.Y2 = BowMan.Y+this.WD.RAD;

		this.R1.X1 = BowMan.X+BowMan.WIDTH;
		this.R1.Y1 = this.R0.Y2;
		this.R1.X2 = BowMan.X+BowMan.WIDTH;
		this.R1.Y2 = BowMan.Y+2*this.WD.RAD;
	...
	render:function(delta){
		...
		// this.mtrl.rope[0].render();
		// this.mtrl.rope[1].render();
		...
		Birds.birds.forEach((val,i,arr)=>{
			...
			if(!isInRect(Dmsns(0,0,this.SCREEN.width,this.SCREEN.height),arrow))
			{
				this.updateBow();
			}
		});
	...
	changeAngle: function(r_angle){
		...
		let rcx = BowMan.X + BowMan.WIDTH/2;
		let rcy = BowMan.Y + BowMan.HEIGHT/3;

		let l0r0 = Math.sqrt(Math.pow(rcx - this.R0.X1,2) + Math.pow(rcy - this.R0.Y1,2));
		let l0r1 = Math.sqrt(Math.pow(rcx - this.R0.X2,2) + Math.pow(rcy - this.R0.Y2,2));

		let l1r0 = Math.sqrt(Math.pow(rcx - this.R1.X1,2) + Math.pow(rcy - this.R1.Y1,2));
		let l1r1 = Math.sqrt(Math.pow(rcx - this.R1.X2,2) + Math.pow(rcy - this.R1.Y2,2));

		this.R0.X1 = this.mtrl.rope[0].x1 = rcx + Math.cos(r_angle)*l0r0;
		this.R0.Y1 = this.mtrl.rope[0].y1 = rcy + Math.sin(r_angle)*l0r0;

		this.R0.X2 = this.mtrl.rope[0].x2 = rcx + Math.cos(r_angle)*l0r1;
		this.R0.Y2 = this.mtrl.rope[0].y2 = rcy + Math.sin(r_angle)*l0r1;

		this.R1.X1 = this.mtrl.rope[1].x1 = rcx + Math.cos(r_angle)*l1r0;
		this.R1.Y1 = this.mtrl.rope[1].y1 = rcy + Math.sin(r_angle)*l1r0;

		this.R1.X2 = this.mtrl.rope[1].x2 = rcx + Math.cos(r_angle)*l1r1;
		this.R1.Y2 = this.mtrl.rope[1].y2 = rcy + Math.sin(r_angle)*l1r1;
	...
	updateBow: function(){
		...
		this.mtrl.rope[0] = new Draw.Line(this.R0.X1,this.R0.Y1,this.R0.X2,this.R0.Y2,this.ROPE_WIDTH,lineCaps.butt,Color("rgba(50,30,50,255)","rgba(50,30,50,255)"));


		this.mtrl.rope[1] = new Draw.Line(this.R1.X1,this.R1.Y1,this.R1.X2,this.R1.Y2,this.ROPE_WIDTH,lineCaps.butt,Color("rgba(50,30,50,255)","rgba(50,30,50,255)"));
...
export const BowMan = {
	...
	init:function(screen){
		this.WIDTH = 15*screen.width/100;
		this.HEIGHT = 8*screen.height/100;
